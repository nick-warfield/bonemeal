...
if (place_empty(x, y) && healthPoints <= hpMax/2.5)
{instance_create(x, y, BloodPrint_obj);}
...
switch(ds_stack_top(StateStack))
{
  ...
  case s_IDLE:      //boss can find target, waiting before taking action
    ...
    if (current_time >= timeStamp[2])
    {
        ...
        {
            sprite_index = Boss_Sweep_Windup_spr;
            
            spd = 0;
            timeStamp[4] = current_time + 1500;     //gives a 1 second wind up to sweep attack
  ...
  case s_ATTACK5:
    ...
    else if (current_time >= timeStamp[4] - 600)
    {
        sprite_index = Boss_Sweep_Winddown_spr;
        if (image_index == image_number - 1) {image_speed = 0;}
    }
    else if (current_time >= timeStamp[4] - 1000)
    {
        image_speed = 15/60;
        sprite_index = Boss_Sweep_Attack_spr;
  
        
        var thing = collision_circle(x, y, 5*32, target, false, false);
